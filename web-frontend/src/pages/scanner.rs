...
pub fn Scanner() -> impl IntoView {
    let (tokens, set_tokens) = create_signal::<Vec<Value>>(Vec::new());
    let (mode, set_mode) = create_signal("trending".to_string());
        ...
        <div class="space-y-4">
            <div class="flex items-center gap-2 mb-4">
                ...
                <div class="flex gap-1 ml-4">
                    {["trending", "trenches"].iter().map(|m| {
                        ...
                        view! {
                            <button
                                class=move || format!("px-3 py-1 rounded text-sm {}", if is_active() { "bg-blue-600 text-white" } else { "bg-gray-800 text-gray-400 hover:bg-gray-700" })
            ...
            <div class="card overflow-x-auto">
                <table class="w-full">
                    ...
                    <tbody>
                        {move || tokens.get().iter().take(30).map(|t| {
                            let symbol = t.get("symbol").and_then(|s| s.as_str()).unwrap_or("???");
                            ...
                            let address = t.get("address").and_then(|a| a.as_str()).unwrap_or("");
                            let path = format!("/analyze/{}", address);
                            ...
                            view! {
                                <tr class="border-b border-gray-800 hover:bg-gray-800 cursor-pointer">
                                    <td class="table-cell font-medium">
                                        <a href=&path class="text-blue-400 hover:underline">{symbol}</a>
                                    </td>
