//! Analyzer page — token detail with alpha report.

use leptos::either::Either;
use leptos::prelude::*;
use leptos::task::spawn_local;
use leptos_router::hooks::use_params_map;
use serde_json::Value;
use crate::api;

#[component]
pub fn Analyzer() -> impl IntoView {
    let params = use_params_map();
    let address = move || params.read().get("address").unwrap_or_default();
    let (data, set_data) = signal::<Option<Value>>(None);

    spawn_local(async move {
        let addr = address();
        if !addr.is_empty() {
            if let Ok(res) = api::analyze_token(&addr).await {
                set_data.set(Some(res));
            }
        }
    });

    view! {
        <div class="space-y-6">
            <h2 class="text-2xl font-bold text-white">"Token Analysis"</h2>

            {move || match data.get() {
                None => Either::Left(view! { <p class="text-gray-500">"Loading token analysis..."</p> }),
                Some(ref d) => Either::Right({
                    let symbol = d.get("detail")
                        .and_then(|x| x.get("symbol"))
                        .and_then(|s| s.as_str())
                        .map(|s| s.to_string())
                        .unwrap_or_else(|| "Unknown".to_string());
                    let name = d.get("detail")
                        .and_then(|x| x.get("name"))
                        .and_then(|n| n.as_str())
                        .map(|n| n.to_string())
                        .unwrap_or_default();
                    let price = d.get("detail")
                        .and_then(|x| x.get("price_usd"))
                        .and_then(|p| p.as_f64())
                        .unwrap_or(0.0);
                    let mc = d.get("detail")
                        .and_then(|x| x.get("market_cap"))
                        .and_then(|m| m.as_f64())
                        .unwrap_or(0.0);
                    let alpha_score = d.get("report")
                        .and_then(|r| r.get("alpha_score"))
                        .and_then(|a| a.as_f64())
                        .unwrap_or(0.0);
                    let rug_severity = d.get("report")
                        .and_then(|r| r.get("rug_report"))
                        .and_then(|rg| rg.get("severity"))
                        .and_then(|s| s.as_str())
                        .map(|s| s.to_string())
                        .unwrap_or_else(|| "Unknown".to_string());
                    let verdict = d.get("report")
                        .and_then(|r| r.get("rug_report"))
                        .and_then(|rg| rg.get("verdict"))
                        .and_then(|v| v.as_str())
                        .map(|v| v.to_string())
                        .unwrap_or_default();
                    let score_color = if alpha_score >= 70.0 {
                        "text-green-400"
                    } else if alpha_score >= 40.0 {
                        "text-yellow-400"
                    } else {
                        "text-red-400"
                    };

                    view! {
                        <div class="grid grid-cols-2 gap-6">
                            <div class="card">
                                <h3 class="text-lg font-semibold text-white mb-2">{symbol} " - " {name}</h3>
                                <p class="text-gray-400">"Price: " <span class="text-white">"$" {format!("{:.8}", price)}</span></p>
                                <p class="text-gray-400">"Market Cap: " <span class="text-white">"$" {format!("{:.0}", mc)}</span></p>
                            </div>
                            <div class="card">
                                <h3 class="text-lg font-semibold text-white mb-2">"Alpha Score"</h3>
                                <p class=format!("text-4xl font-bold {}", score_color)>{format!("{:.0}", alpha_score)}</p>
                            </div>
                        </div>
                        <div class="card">
                            <h3 class="text-lg font-semibold text-white mb-2">"Rug Report"</h3>
                            <div class="flex items-center gap-2">
                                <span class=format!("badge {}", if rug_severity == "Low" { "badge-green" } else if rug_severity == "Medium" { "badge-yellow" } else { "badge-red" })>
                                    {rug_severity.clone()}
                                </span>
                                <span class="text-gray-400 text-sm">{verdict}</span>
                            </div>
                        </div>
                    }
                }),
            }}
        </div>
    }
}
