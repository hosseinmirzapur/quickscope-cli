...
pub async fn run_auto_tune(pool: &SqlitePool) -> Result<Option<TuneResult>> {
    ...
    let winners: Vec<_> = closed.iter()
        .filter(|p| p.pnl_sol.unwrap_or(0.0) > 0.0)
        .collect();
    let losers: Vec<_> = closed.iter()
        .filter(|p| p.pnl_sol.unwrap_or(0.0) <= 0.0)
        .collect();
    ...
    let winner_fvs: Vec<_> = winners.iter()
        .filter_map(|p| {
            p.feature_vector.as_ref()
                .and_then(|json| serde_json::from_str::<crate::data::models::FeatureVector>(json).ok())
        })
        .collect();
    let loser_fvs: Vec<_> = losers.iter()
        .filter_map(|p| {
            p.feature_vector.as_ref()
                .and_then(|json| serde_json::from_str::<crate::data::models::FeatureVector>(json).ok())
        })
...
fn apply_weight_nudges(
    ...
    let nudges: Vec<(&mut f64, f64)> = vec![
        ...
        (&mut config.w_safety, category_map.get("safety").copied().unwrap_or(0.0)),
        (&mut config.w_holder, category_map.get("holder").copied().unwrap_or(0.0)),
        (&mut config.w_liquidity, category_map.get("liquidity").copied().unwrap_or(0.0)),
        (&mut config.w_dev, category_map.get("dev").copied().unwrap_or(0.0)),
        (&mut config.w_social, category_map.get("social").copied().unwrap_or(0.0)),
    ];
...
fn build_category_map(discriminations: &[FeatureDiscrimination]) -> std::collections::HashMap<String, f64> {
    let mut map: std::collections::HashMap<String, Vec<f64>> = std::collections::HashMap::new();

    for d in discriminations {
        let category = if d.feature_name.starts_with("momentum") { "momentum" }
        else if d.feature_name.starts_with("safety") { "safety" }
        else if d.feature_name.starts_with("holder") { "holder" }
        else if d.feature_name.starts_with("liquidity") { "liquidity" }
        else if d.feature_name.starts_with("dev") { "dev" }
        else if d.feature_name.starts_with("social") || d.feature_name.starts_with("twitter") { "social" }
        else { continue };
    ...
    fn test_category_map_building() {
        let disc = vec![
            FeatureDiscrimination {
                ...
                winner_mean: 100_000.0, loser_mean: 10_000.0,
                winner_std: 10.0, loser_std: 10.0,
                discrimination_power: 3.0,
                recommended_direction: "increase".to_string(),
            },
            FeatureDiscrimination {
                feature_name: "safety_rug_ratio".to_string(),
                winner_mean: 0.05, loser_mean: 0.40,
                winner_std: 0.01, loser_std: 0.01,
