...
fn render_token_list(frame: &mut Frame, area: Rect, state: &AppState, theme: &Theme) {
    let items: Vec<ListItem> = if state.trending.is_empty() {
        ...
        state.trending.iter().enumerate().map(|(i, t)| {
            let sel = i == state.list_cursor;
            ...
            ListItem::new(Line::from(vec![
                ...
                Span::raw(" "),
                Span::styled(format!("${:.8}", t.price_usd), s),
                Span::raw(" "),
                Span::styled(format!("MC ${:.0}", t.market_cap), Style::default().fg(theme.muted)),
                Span::raw(" "),
                Span::styled(format!("{:+.1}%", chg), Style::default().fg(cc)),
                Span::raw(" "),
                Span::styled(format!("Liq ${:.0}", t.liquidity_usd), Style::default().fg(theme.muted)),
                if sm > 0 { Span::styled(format!(" SM:{}", sm), Style::default().fg(theme.accent)) } else { Span::raw("") },
            ]))
        }).collect()
    };
...
fn render_detail_panel(frame: &mut Frame, area: Rect, state: &AppState, theme: &Theme) {
    if let Some(ref detail) = state.selected_token {
        ...
        let text = Line::from(vec![
            ...
            Span::raw(format!(" | MC: ${:.0}", t.market_cap)),
