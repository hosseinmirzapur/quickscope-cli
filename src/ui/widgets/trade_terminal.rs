...
fn render_order_form(frame: &mut Frame, area: Rect, state: &AppState, theme: &Theme) {
    let mut lines = vec![
        ...
        Line::from(""),
        Line::from(Span::raw(
            " Amount:  [____0.5 SOL____]   Slippage: [__3%__]",
        )),
        Line::from(Span::raw(" TP:      [___100%____]   SL:      [__60%__]")),
        Line::from(""),
        Line::from(Span::styled(
            " Modes: [EXPLODE] [ALPHA] [SCALP] [FALLBACK]",
    ...
    if let Some(ref token) = state.selected_token {
        lines.push(Line::from(Span::styled(
            format!(
                " Price: ${:.8} | Liq: ${:.0}",
                token.token.price_usd, token.token.liquidity_usd
            ),
        ...
        state
            ...
            .map(|pos| {
                ...
                ListItem::new(Line::from(vec![
                    ...
                    Span::styled(
                        format!(" {}", pos.token_symbol),
                        s.add_modifier(Modifier::BOLD),
                    ),
                    Span::raw(format!(" @ ${:.6}", pos.entry_price)),
                    Span::raw(format!(" {} SOL", pos.amount_sol)),
                    Span::styled(
                        format!(" {} ", pos.mode.as_str()),
                        Style::default().fg(theme.warning),
                    ),
                    Span::styled(format!(" PnL:{}", pnl_str), Style::default().fg(pc)),
                    if let Some(tp) = pos.tp_percent {
                        Span::raw(format!(" TP:{:.0}%", tp))
                    } else {
                        Span::raw("")
                    },
                    if let Some(sl) = pos.sl_percent {
                        Span::raw(format!(" SL:{:.0}%", sl))
                    } else {
                        Span::raw("")
                    },
                ]))
...
fn render_risk_bar(frame: &mut Frame, area: Rect, state: &AppState, theme: &Theme) {
    ...
    let ks_color = if r.kill_switch_active {
        theme.danger
    } else {
        theme.success
    };
    let ks_text = if r.kill_switch_active {
        "⚠ KILL SWITCH ACTIVE"
    } else {
        "Kill Switch: OFF"
    };
    ...
    let text = Line::from(vec![
        ...
        Span::styled(
            format!(
                "Daily: {:+.2}/{} SOL",
                r.daily_realized_pnl, r.daily_loss_cap_sol
            ),
            Style::default().fg(theme.fg),
        ),
        Span::raw(" │ "),
        Span::styled(
            format!(
                "Trades: {} ({}W/{}L)",
                r.trades_today, r.wins_today, r.losses_today
            ),
            Style::default().fg(theme.fg),
        ),
        Span::raw(" │ "),
