...
impl Widget for Chart {
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        let start = self
            .scroll
            .min(self.candles.len().saturating_sub(area.width as usize));
        ...
        for (i, c) in visible.iter().enumerate() {
            ...
            // High-low line
            let hi_y =
                area.y + ((self.max_price - c.high) / range * (area.height as f64)).round() as u16;
            let lo_y =
                area.y + ((self.max_price - c.low) / range * (area.height as f64)).round() as u16;
            ...
            // Open (left tick)
            let open_y =
                area.y + ((self.max_price - c.open) / range * (area.height as f64)).round() as u16;
            let open_y = open_y.min(area.y + area.height - 1);
            buf.set_string(x.saturating_sub(1), open_y, "┤", style);

            // Close (right tick)
            let close_y =
                area.y + ((self.max_price - c.close) / range * (area.height as f64)).round() as u16;
