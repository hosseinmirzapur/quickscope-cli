...
fn render_portfolio_panel(frame: &mut Frame, area: Rect, state: &AppState, theme: &Theme) {
    ...
    let mut lines = vec![
        ...
        Line::from(vec![
            Span::raw("Balance:  "),
            Span::styled(
                format!("{:.2} SOL", state.balance_sol),
        ...
        Line::from(vec![
            Span::raw(format!(
                "Trades: {}W/{}L",
                state.risk_state.wins_today, state.risk_state.losses_today
            )),
        ...
        Line::from(vec![
            ...
            Span::styled(
                ...
                Style::default().fg(if state.risk_state.kill_switch_active {
                    theme.danger
                } else {
                    theme.muted
                }),
...
fn render_trending_panel(frame: &mut Frame, area: Rect, state: &AppState, theme: &Theme) {
    ...
    // Clamp scroll offset to valid range — prevents items rendering outside visible area
    ...
    let visible_tokens: Vec<&&TrendingToken> =
        filtered.iter().skip(offset).take(visible_rows).collect();
